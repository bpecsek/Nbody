...
(declaim (ftype (function ((simple-array double-float (*))
			   (integer 0 #.most-positive-fixnum))
			  (simd-pack-256 double-float)) d4-ref))
(defun d4-ref (v i)
  (declare (optimize (speed 3) (safety 0) (debug 0))
...
(declaim (ftype (function ((simd-pack-256 double-float)
			   ...
			   (integer 0 #.most-positive-fixnum))
			  (simd-pack-256 double-float)) (setf d4-ref)))
(defun (setf d4-ref) (new-value v i)
  (declare (optimize (speed 3) (safety 0) (debug 0))
	   ((simple-array double-float (*)) v)
	   ((integer 0 #.most-positive-fixnum) i)
...
(define-constant %1.0d4c (%make-avx-double 1.0d0 1.0d0 1.0d0 1.0d0))
(define-constant +dt+  0.01d0)
(define-constant +recip_dt+ (/ 1.0d0 +dt+))
(declaim (type fixnum +bodies_count+ +interactions_count+ +ric+)
	 (type (double-float 0.0d0) +dt+ +recip_dt+))
  ...
  (let ((pdx *pdx*)
    ...
    (loop with i of-type fixnum = 0
      ...
      (loop for i of-type fixnum below +ric+ by 4
	    do (let* ((%dx (d4-ref pdx i))
		      (%dy (d4-ref pdy i))
		      (%dz (d4-ref pdz i))
		      (%posdelta_sq (d4+ (d4+ (d4* %dx %dx) (d4* %dy %dy))
					 (d4* %dz %dz)))
		      (%dist (d4sqrt %posdelta_sq))
		      (%mag (d4/ %1.0d4c (d4* %dist %posdelta_sq))))
		 (declare ((simd-pack-256 double-float) %dx %dy %dz)
			  ((simd-pack-256 (double-float 0.0d0)) %posdelta_sq %dist %mag)
			  (position_deltas pdx pdy pdz))
		 (setf (d4-ref mag i) %mag))
	    (vzeroupper)) 
      (loop with i of-type fixnum = 0
	    for (a . rest) on system
	    do (dolist (b rest)
