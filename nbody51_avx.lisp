       ...
       ;; interaction.
	     ...
	     ;; which is why ROUNDED_INTERACTIONS_COUNT/2 iterations are done.
             (loop for i of-type fixnum below +ROUNDED_INTERACTIONS_COUNT+ by 4
              do (let* ((%pdx (d4ref position_Deltas-x i))
                        (%pdy (d4ref position_Deltas-y i))
                        (%pdz (d4ref position_Deltas-z i))
                        (%distance_Squared (d4+ (d4* %pdx %pdx) (d4* %pdy %pdy)
                                           (d4* %pdz %pdz)))
                        (%magnitude (d4/ (d4* (d4sqrt %distance_Squared)
                                         %distance_Squared))))
                   (setf (d4ref magnitudes i) %magnitude)))
	     (vzeroupper))
           (velocities ()
